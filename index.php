<?php
/*
Plugin Name: LWD 125x125 Ad Widget
Plugin URI: http://smallbizwebdesigns.co.uk
Description: This is a customizable widget which will displays the 125 x 125 ad unit (maximum 6 ads).
...
function lwdad_widget_styles() {
    wp_enqueue_style( 'add-style', plugins_url( 'css/style.css', __FILE__ ) );
}

add_action( 'wp_enqueue_scripts', 'lwdad_widget_styles' );

// load widget
add_action( 'widgets_init', 'lwd_ads_widgets' );

// Register widget
function lwd_ads_widgets() {
	register_widget( 'lwd_ad_widgets' );
}

// Widget class
class lwd_ad_widgets extends WP_Widget {
...
function lwd_ad_widgets() {

	// Widget settings
	$widget_ops = array (

		'classname' => 'lwd_ad_widgets',
	...
	$control_ops = array (
		...
		'id_base' => 'lwd_ad_widgets'

	);

	// Create the widget
	$this->WP_Widget( 'lwd_ad_widgets', __('LWD:125x125 Ads', 'framework'), $widget_ops, $control_ops );
...
function lwd_update( $new_instance, $old_instance ) {
...
function form( $instance ) {
	...
	$defaults = array(
		...
		'ad1' => plugin_dir_url( __FILE__ ) . "images/add-125x125.gif",

		'link1' => '#',

		'ad2' => plugin_dir_url( __FILE__ )."images/add-125x125.gif",
